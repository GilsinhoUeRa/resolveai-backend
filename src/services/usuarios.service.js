...
const getMe = async (userId) => {
    return await db.selectFrom('usuarios as u')
        ...
        .selectAll('u')
        .select(['pp.bio', 'pp.whatsapp', 'pp.horario_atendimento', 'p.nome as profession_name'])
        .where('u.id', '=', userId)
        .executeTakeFirst();
};

const update = async (targetUserId, updateData) => {
    // Kysely ignora campos 'undefined' automaticamente, tornando a query mais limpa.
...
const becomeProvider = async (userId, providerData) => {
    ...
    // Kysely tem um gestor de transações seguro e moderno.
